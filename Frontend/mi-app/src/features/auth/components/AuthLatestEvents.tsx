...
function EventSkeleton() {
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-white/20 bg-white/12 p-3 backdrop-blur-md">
      <div className="h-11 w-11 shrink-0 animate-pulse rounded-xl bg-white/25" />
      <div className="min-w-0 flex-1 space-y-2">
        <div className="h-3 w-3/4 animate-pulse rounded bg-white/25" />
        <div className="h-2.5 w-1/2 animate-pulse rounded bg-white/15" />
      </div>
    </div>
  );
}
    ...
    <div className="space-y-3">
      {events.map((event) => {
        ...
        return (
          <div
            key={event.id}
            className="flex items-center gap-3 rounded-2xl border border-white/20 bg-white/12 p-3 backdrop-blur-md"
          >
            <div className="flex h-11 w-11 shrink-0 flex-col items-center justify-center rounded-xl bg-white/20 text-white ring-1 ring-white/25">
